...
The first thing to note is that Plex Meta Manager utilizes the ASCII sort order, which defines which priority a character gets.

![](images\sorts.jpg)

### Collection Sorting

Collections can be sorted so that they appear in a specific order in the `Collections` tab of your library and within the `Library` tab. This is done using the `sort_title` attribute.
...
#### PMM Default Collection Sorting

When utilizing the [PMM Defaults](../../defaults/files.md), they are all built on the same format for their sort titles:
...
???+ example "Example 3 - Reordering PMM Defaults"
...
You can also re-order collections within each Defaults file using the `order_<<key>>` template variable. <<key>> refers to the identifier that PMM uses for each collection, which you can find on the relative default's wiki page.

???+ example "Example 4 - Reordering Collections within a PMM Defaults file"
...
When using PMM builders, the majority of builders will require `collection_order: custom` to be set, which allows you to sort either by a sort order that the builder source allows for (such as sorting an MDBList by revenue), or by the default order that PMM receives the items from the builder (such as sorting the IMDb Top 250 in the order that they appear on the list).
...
When using Plex Meta Manager, a "Dumb" collection is one which is static - the collection will only update when you run Plex Meta Manager.
