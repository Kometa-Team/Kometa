...
| `default_percent`            | **Description:** Used to declare the default percent threshold for explicit `episodes`, `seasons`, `tracks`, and `albums` [special filters](filters.md#special-filters) when neither `percentage` nor `count` is specified. Note: sub-filters such as `audio_track_title`, `subtitle_language`, `audio_language`, `filepath`, `resolution`, `subtitle_track_title`, and `has_dolby_vision` are automatically wrapped with `count: 1` (any matching episode/track is sufficient) and are not affected by this setting. See [Example](#default-percent-example) below.<br>**Default:** `50`.<br>**Values:** Integer between 1 and 100                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
An example of using `default_percent` with an explicit `episodes` special filter to require at least 35% of a show's episodes to match before the show is included:
