...
| `server_preroll`             | **Description:** Used to set the `Movie pre-roll video` Text box in Plex under Settings -> Extras.<br>You can run this with a [schedule](../config/schedule.md) to change the pre-rolls automatically.<br>**Values:** Any String or a List of Strings to be separated by commas `,` (A nested list of strings in the original list will output the nested items separated by semi-colons `;`)                                                                                                                             |
