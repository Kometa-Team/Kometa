...
| Monthly      | Update once a month on the specified day or inclusive day range. Use `last` as the range end to include the final day of every month regardless of its length. | monthly(Day of Month)<br>monthly(Start Day-End Day)<br>monthly(Start Day-last)<br>monthly(last) | `monthly(1)`<br>`monthly(1-7)`<br>`monthly(21-last)`<br>`monthly(last)` |
