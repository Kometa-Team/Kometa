...
??? blank "`auto_sort_hubs` - Used to sort Recommendation Hub rows on the Plex home screen.<a class="headerlink" href="#auto-sort-hubs" title="Permanent link">¶</a>"
    ...
    Hub sorting is skipped entirely on a targeted [`--run-collections`](../kometa/environmental.md#run-collections) run, since only the requested collections' `hub_priority` values are known that run; run without `--run-collections` to re-apply the full sort.
