...
???+ tip
    ...
    MyAnimeList may may specific requirements for any value shown below, which may change over time, so refer to the form at MyAnimeList in the event that any of these example values do not work.
...
4. Enter an `App Description` for the application Ex. `Kometa manages metadata and collections on Plex servers`
