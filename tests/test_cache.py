...
def test_overlay_value_cache_force_expire_stores_today(cache):
    # expired=True resets the clock to today (no jitter), giving the entry the maximum freshness window.
    # Verify the stored date is today — not a randomly-jittered past date.
    ...
    with sqlite3.connect(cache.cache_path) as connection:
        row = connection.execute(
            "SELECT expiration_date FROM overlay_value_cache WHERE rating_key='5173' AND type='plex_imdb_rating'"
        ).fetchone()
    assert row[0] == datetime.now().strftime("%Y-%m-%d")


def test_migrate_overlay_value_cache_multiple_libraries(cache):
...
def test_migrate_overlay_value_cache_idempotent(cache):
