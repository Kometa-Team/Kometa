...
def test_schedule_not_inverts_each_rule_independently():
    ...
    assert util.schedule_check("test", "weekly.not(monday)", monday, 12) == ""

    with pytest.raises(NotScheduled):
        util.schedule_check("test", "weekly.not(monday)", datetime(2026, 6, 16, 12, 0), 12)

    # Rules remain ORed after an individual modifier is evaluated.
    assert util.schedule_check("test", ["weekly.not(monday)", "weekly(tuesday)"], monday, 12) == ""
...
def test_library_schedule_modes_select_the_expected_items(monkeypatch):
    ...
    library.config = SimpleNamespace(run_hour=12)
    library.scheduled_cached_keys = {2}
    items = [
    ...
    assert library._schedule_item_keys(items) == {1, 3}
