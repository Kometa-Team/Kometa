    ...
    def run_operations(self):
        ...
        if self.library.items_library_operation:
            ...
            for i, item in enumerate(items, 1):
                ...
                if self.library.mass_poster_update or self.library.mass_background_update:
                    try:
                        new_poster, new_background, _logo, _square, item_dir, name = self.library.find_item_assets(item)  # noqa: F841
                    except Failed:
                        new_poster, new_background, _logo, _square, item_dir, name = None, None, None, None, None, None  # noqa: F841
                    try:
                        tmdb_item = tmdb_obj()
                    except Failed:
                        tmdb_item = None
                        ...
                        try:
                            ...
                            for season in self.library.query(item.seasons):
                                ...
                                if (self.library.mass_poster_update and self.library.mass_poster_update["seasons"]) or (self.library.mass_background_update and self.library.mass_background_update["seasons"]):
                                    try:
                                        season_poster, season_background, _season_logo, _season_square, _, _ = self.library.find_item_assets(season, item_asset_directory=item_dir, folder_name=name)
                                    except Failed:
                                        season_poster = None
                                ...
                                if (self.library.mass_poster_update and self.library.mass_poster_update["episodes"]) or (self.library.mass_background_update and self.library.mass_background_update["episodes"]):
                                    ...
                                    for episode in self.library.query(season.episodes):
                                        try:
                                            ...
                                            continue
                                        try:
                                            episode_poster, episode_background, _episode_logo, _episode_square, _, _ = self.library.find_item_assets(episode, item_asset_directory=item_dir, folder_name=name)
                                        except Failed:
                                            episode_poster = None
        ...
        if self.library.delete_collections or self.library.show_unmanaged or self.library.show_unconfigured or self.library.assets_for_all or self.library.mass_collection_mode:
            ...
            if self.library.assets_for_all_collections and len(unconfigured_collections) > 0:
                ...
                for col in unconfigured_collections:
                    try:
                        poster, background, logo, square, item_dir, name = self.library.find_item_assets(col)
                        if poster or background or logo or square:
                            self.library.upload_images(col, poster=poster, background=background, logo=logo, square=square)
                        elif self.library.show_missing_assets:
                            logger.warning(f"Asset Warning: No supported artwork found in an assets folder for '{name}'")
                    except Failed as e:
                        logger.warning(e)
