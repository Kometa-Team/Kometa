...
class BuilderAttributeSetter:
    ...
    def _poster(self, method_name, method_data):
        if method_name == "url_poster":
            try:
                ...
                self.logger.warning(f"{self.collectionBuilder.Type} Warning: No Poster Found at {method_data}")
            ...
            if os.path.exists(os.path.abspath(method_data)):
                ...
                self.logger.error(f"{self.collectionBuilder.Type} Error: Poster Path Does Not Exist: {os.path.abspath(method_data)}")
    ...
    def _background(self, method_name, method_data):
        if method_name == "url_background":
            try:
                ...
                self.logger.warning(f"{self.collectionBuilder.Type} Warning: No Background Found at {method_data}")
            ...
            if os.path.exists(os.path.abspath(method_data)):
                ...
                self.logger.error(f"{self.collectionBuilder.Type} Error: Background Path Does Not Exist: {os.path.abspath(method_data)}")
    ...
    def _details(self, method_name, method_data, method_final, methods):
        if method_name == "url_theme":
            ...
            if os.path.exists(os.path.abspath(method_data)):
                ...
                self.logger.error(f"{self.collectionBuilder.Type} Error: Theme Path Does Not Exist: {os.path.abspath(method_data)}")
        elif method_name == "tmdb_region":
            self.collectionBuilder.tmdb_region = util.parse(self.collectionBuilder.Type, method_name, method_data, options=self.collectionBuilder.config.TMDb.iso_3166_1)
            ...
            self.collectionBuilder.minimum = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", minimum=1)
        elif method_name == "cache_builders":
            self.collectionBuilder.details[method_name] = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", minimum=0)
        elif method_name == "default_percent":
            self.collectionBuilder.default_percent = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", minimum=1, maximum=100)
        elif method_name == "server_preroll":
            self.collectionBuilder.server_preroll = util.parse(self.collectionBuilder.Type, method_name, method_data)
        elif method_name == "ignore_ids":
            self.collectionBuilder.ignore_ids.extend(util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="intlist"))
        elif method_name == "ignore_imdb_ids":
            self.collectionBuilder.ignore_imdb_ids.extend(util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="list"))
        elif method_name == "label":
            if "label" in methods and "label.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use label and label.sync together")
            if "label.remove" in methods and "label.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use label.remove and label.sync together")
            ...
            self.collectionBuilder.details[method_name] = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="list") if method_data else None
            ...
            if isinstance(method_data, bool):
                ...
                try:
                    util.schedule_check(method_name, util.parse(self.collectionBuilder.Type, method_name, method_data), self.collectionBuilder.current_time, self.collectionBuilder.config.run_hour)
                    self.collectionBuilder.details[method_name] = True
                except NotScheduled:
                    self.collectionBuilder.details[method_name] = False
            ...
            self.collectionBuilder.details[method_name] = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="bool", default=default)
    ...
    def _item_details(self, method_name, method_data, method_mod, method_final, methods):
        if method_name == "item_label":
            if "item_label" in methods and "item_label.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use item_label and item_label.sync together")
            if "item_label.remove" in methods and "item_label.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use item_label.remove and item_label.sync together")
        ...
        if method_name == "item_genre":
            if "item_genre" in methods and "item_genre.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use item_genre and item_genre.sync together")
            if "item_genre.remove" in methods and "item_genre.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use item_genre.remove and item_genre.sync together")
            self.collectionBuilder.item_details[method_final] = util.get_list(method_data) if method_data else []
            ...
            if not method_data:
                raise Failed(f"{self.collectionBuilder.Type} Error: non_item_remove_label is blank")
            ...
            if method_name in methods and f"{method_name}.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use {method_name} and {method_name}.sync together")
            if f"{method_name}.remove" in methods and f"{method_name}.sync" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use {method_name}.remove and {method_name}.sync together")
            if method_name in methods and f"{method_name}.remove" in methods:
                raise Failed(f"{self.collectionBuilder.Type} Error: Cannot use {method_name} and {method_name}.remove together")
            ...
            self.collectionBuilder.item_details[method_name] = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", default=0, minimum=0)
        elif method_name in item_bool_details:
            if util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="bool", default=False):
    ...
    def _radarr(self, method_name, method_data):
        if method_name in ["radarr_add_missing", "radarr_add_existing", "radarr_upgrade_existing", "radarr_monitor_existing", "radarr_search", "radarr_monitor", "radarr_ignore_cache"]:
            self.collectionBuilder.radarr_details[method_name[7:]] = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="bool")
            ...
            if str(method_data).lower() in radarr.availability_translation:
                ...
                raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} attribute must be either announced, cinemas, released or db")
    ...
    def _sonarr(self, method_name, method_data):
        if method_name in ["sonarr_add_missing", "sonarr_add_existing", "sonarr_upgrade_existing", "sonarr_monitor_existing", "sonarr_season", "sonarr_search", "sonarr_cutoff_search", "sonarr_ignore_cache"]:
            self.collectionBuilder.sonarr_details[method_name[7:]] = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="bool")
            ...
            if str(method_data).lower() in sonarr.monitor_translation:
                ...
                raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} attribute must be either all, future, missing, existing, pilot, first, latest or none")
            ...
            if str(method_data).lower() in sonarr.series_types:
                ...
                raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} attribute must be either standard, daily, or anime")
    ...
    def _anidb(self, method_name, method_data):
        if method_name == "anidb_popular":
            self.collectionBuilder.builders.append((method_name, util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", default=30, maximum=30)))
            ...
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                dict_methods = {dm.lower(): dm for dm in dict_data}
                new_dictionary = {}
                if "tag" not in dict_methods:
                    raise Failed(f"{self.collectionBuilder.Type} Error: anidb_tag tag attribute is required")
                ...
                new_dictionary["limit"] = util.parse(self.collectionBuilder.Type, "limit", dict_data, datatype="int", methods=dict_methods, default=0, parent=method_name, minimum=0)
    ...
    def _anilist(self, method_name, method_data):
        if method_name in ["anilist_id", "anilist_relations", "anilist_studio"]:
            ...
            self.collectionBuilder.builders.append((method_name, util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", default=10)))
        elif method_name == "anilist_userlist":
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                dict_methods = {dm.lower(): dm for dm in dict_data}
                new_dictionary = {
                    "username": util.parse(self.collectionBuilder.Type, "username", dict_data, methods=dict_methods, parent=method_name),
                    "list_name": util.parse(self.collectionBuilder.Type, "list_name", dict_data, methods=dict_methods, parent=method_name),
                    "sort_by": util.parse(self.collectionBuilder.Type, "sort_by", dict_data, methods=dict_methods, parent=method_name, default="score", options=anilist.userlist_sort_options),
                }
                score_dict = {}
                for search_method, search_data in dict_data.items():
                    ...
                    if search_attr == "score" and modifier in [".gt", ".gte", ".lt", ".lte"]:
                        score = util.parse(self.collectionBuilder.Type, search_method, dict_data, methods=dict_methods, datatype="int", default=-1, minimum=0, maximum=10, parent=method_name)
                        if score > -1:
                            score_dict[modifier] = score
                    elif search_attr not in ["username", "list_name", "sort_by"]:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {search_method} attribute not supported")
            ...
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                ...
                for search_method, search_data in dict_data.items():
                    ...
                    if lower_method not in anilist.searches:
                        ...
                        new_dictionary[search_attr] = util.parse(self.collectionBuilder.Type, search_attr, search_data, parent=method_name, default=current_season, options=util.seasons)
                        ...
                        new_dictionary[search_attr] = util.parse(self.collectionBuilder.Type, search_attr, search_data, datatype="int", parent=method_name, default=default_year, minimum=1917, maximum=default_year + 1)
                    elif search_data is None:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {search_method} attribute is blank")
                    elif search_attr == "adult":
                        new_dictionary[search_attr] = util.parse(self.collectionBuilder.Type, search_attr, search_data, datatype="bool", parent=method_name)
                    elif search_attr == "country":
                        new_dictionary[search_attr] = util.parse(self.collectionBuilder.Type, search_attr, search_data, options=anilist.country_codes, parent=method_name)
                    elif search_attr == "source":
                        new_dictionary[search_attr] = util.parse(self.collectionBuilder.Type, search_attr, search_data, options=anilist.media_source, parent=method_name)
                    elif search_attr in ["episodes", "duration", "score", "popularity"]:
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="int", parent=method_name)
                    elif search_attr in ["format", "status", "genre", "tag", "tag_category"]:
                        new_dictionary[lower_method] = self.collectionBuilder.config.AniList.validate(search_attr.replace("_", " ").title(), util.parse(self.collectionBuilder.Type, search_method, search_data))
                    elif search_attr in ["start", "end"]:
                        new_dictionary[search_attr] = util.parse(self.collectionBuilder.Type, search_attr, search_data, datatype="date", parent=method_name, date_return="%m/%d/%Y")
                    elif search_attr == "min_tag_percent":
                        new_dictionary[search_attr] = util.parse(self.collectionBuilder.Type, search_attr, search_data, datatype="int", parent=method_name, minimum=0, maximum=100)
                ...
                if len(new_dictionary) == 0:
                    raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} must have at least one valid search option")
                new_dictionary["sort_by"] = util.parse(self.collectionBuilder.Type, "sort_by", dict_data, methods=dict_methods, parent=method_name, default="score", options=anilist.sort_options)
                new_dictionary["limit"] = util.parse(self.collectionBuilder.Type, "limit", dict_data, datatype="int", methods=dict_methods, default=0, parent=method_name)
                self.collectionBuilder.builders.append((method_name, new_dictionary))
        ...
        if method_name == "imdb_id":
            for value in util.get_list(method_data):
                if str(value).startswith("tt"):
                    ...
                    raise Failed(f"{self.collectionBuilder.Type} Error: imdb_id {value} must begin with tt")
        elif method_name == "imdb_list":
            try:
                for imdb_dict in self.collectionBuilder.config.IMDb.validate_imdb_lists(self.collectionBuilder.Type, method_data, self.collectionBuilder.language):
            ...
            for value in util.get_list(method_data):
                if value in imdb.movie_charts and not self.collectionBuilder.library.is_movie:
                    raise Failed(f"{self.collectionBuilder.Type} Error: chart: {value} does not work with show libraries")
                elif value in imdb.show_charts and self.collectionBuilder.library.is_movie:
                    raise Failed(f"{self.collectionBuilder.Type} Error: chart: {value} does not work with movie libraries")
                elif value in imdb.movie_charts or value in imdb.show_charts:
                    self.collectionBuilder.builders.append((method_name, value))
                else:
                    raise Failed(f"{self.collectionBuilder.Type} Error: chart: {value} is invalid options are {[i for i in imdb.charts]}")
        elif method_name == "imdb_watchlist":
            for imdb_user in self.collectionBuilder.config.IMDb.validate_imdb_watchlists(self.collectionBuilder.Type, method_data, self.collectionBuilder.language):
                self.collectionBuilder.builders.append((method_name, imdb_user))
        elif method_name == "imdb_award":
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                dict_methods = {dm.lower(): dm for dm in dict_data}
                event_id = util.parse(self.collectionBuilder.Type, "event_id", dict_data, parent=method_name, methods=dict_methods, regex=(r"(ev\d+)", "ev0000003"))
                git_event, year_options = self.collectionBuilder.config.IMDb.get_event_years(event_id)
                if not year_options:
                    raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award event_id attribute: No event found at {imdb.base_url}/event/{event_id}")
                if "event_year" not in dict_methods:
                    raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award event_year attribute not found")
                og_year = dict_data[dict_methods["event_year"]]
                if not og_year:
                    raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award event_year attribute is blank")
                if og_year in ["all", "latest"]:
                    ...
                    try:
                        ...
                        raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award event_year attribute invalid: {og_year}")
                else:
                    event_year = util.parse(self.collectionBuilder.Type, "event_year", og_year, parent=method_name, datatype="strlist", options=year_options)
                if (event_year == "all" or len(event_year) > 1) and not git_event:
                    raise Failed(f"{self.collectionBuilder.Type} Error: Only specific events work when using multiple years. Event Options: [{', '.join([k for k in self.collectionBuilder.config.IMDb.events_validation])}]")
                award_filters = []
                if "award_filter" in dict_methods:
                    if not dict_data[dict_methods["award_filter"]]:
                        raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award award_filter attribute is blank")
                    award_filters = util.parse(self.collectionBuilder.Type, "award_filter", dict_data[dict_methods["award_filter"]], datatype="lowerlist")
                category_filters = []
                if "category_filter" in dict_methods:
                    if not dict_data[dict_methods["category_filter"]]:
                        raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award category_filter attribute is blank")
                    category_filters = util.parse(self.collectionBuilder.Type, "category_filter", dict_data[dict_methods["category_filter"]], datatype="lowerlist")
                final_category = []
                final_awards = []
                if award_filters or category_filters:
                    ...
                    for award_filter in award_filters:
                        if award_filter in lower_award:
                            ...
                            raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award award_filter attribute invalid: {award_filter} must be in in [{', '.join([v for _, v in lower_award.items()])}]")
                    ...
                    for category_filter in category_filters:
                        if category_filter in lower_category:
                            ...
                            raise Failed(f"{self.collectionBuilder.Type} Error: imdb_award category_filter attribute invalid: {category_filter} must be in in [{', '.join([v for _, v in lower_category.items()])}]")
                self.collectionBuilder.builders.append((method_name, {
                    ...
                    "winning": util.parse(self.collectionBuilder.Type, "winning", dict_data, parent=method_name, methods=dict_methods, datatype="bool", default=False)
                }))
        elif method_name == "imdb_search":
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                dict_methods = {dm.lower(): dm for dm in dict_data}
                new_dictionary = {"limit": util.parse(self.collectionBuilder.Type, "limit", dict_data, datatype="int", methods=dict_methods, minimum=0, default=100, parent=method_name)}
                for search_method, search_data in dict_data.items():
                    lower_method = str(search_method).lower()
                    search_attr, modifier = os.path.splitext(lower_method)
                    if search_data is None:
                        ...
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, parent=method_name, options=imdb.sort_options)
                    elif search_attr == "title":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, parent=method_name)
                    elif search_attr == "type":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name, options=imdb.title_type_options)
                    elif search_attr == "topic":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name, options=imdb.topic_options)
                    elif search_attr == "release":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="date", parent=method_name, date_return="%Y-%m-%d")
                    elif search_attr == "rating":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="float", parent=method_name, minimum=0.1, maximum=10)
                    elif search_attr in ["votes", "imdb_top", "imdb_bottom", "popularity", "runtime"]:
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="int", parent=method_name, minimum=0)
                    elif search_attr == "genre":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name, options=imdb.genre_options)
                    elif search_attr == "event":
                        events = []
                        for event in util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name):
                        ...
                        companies = []
                        for company in util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name):
                        ...
                        countries = []
                        for country in util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="upperlist", parent=method_name):
                            if country:
                        ...
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name)
                    elif search_attr == "cast":
                        casts = []
                        for cast in util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name):
                        ...
                        series = []
                        for show in util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name):
                        ...
                        lists = []
                        for new_list in util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="lowerlist", parent=method_name):
                        ...
                        if util.parse(self.collectionBuilder.Type, search_method, search_data, datatype="bool", parent=method_name):
                ...
                if len(new_dictionary) > 1:
                    ...
                    raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} had no valid fields")

    def _letterboxd(self, method_name, method_data):
        if method_name.startswith("letterboxd_list"):
            letterboxd_lists = self.collectionBuilder.config.Letterboxd.validate_letterboxd_lists(self.collectionBuilder.Type, method_data, self.collectionBuilder.language)
    ...
    def _mal(self, method_name, method_data):
        if method_name == "mal_id":
            ...
            self.collectionBuilder.builders.append((method_name, util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", default=10, maximum=100 if method_name == "mal_suggested" else 500)))
            ...
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                ...
                if method_name == "mal_season":
                    ...
                    season = util.parse(self.collectionBuilder.Type, "season", dict_data, methods=dict_methods, parent=method_name, default=default_season, options=util.seasons)
                    if season == "current":
                        season = default_season
                    self.collectionBuilder.builders.append((method_name, {
                        "season": season,
                        "sort_by": util.parse(self.collectionBuilder.Type, "sort_by", dict_data, methods=dict_methods, parent=method_name, default="members", options=mal.season_sort_options, translation=mal.season_sort_translation),
                        "year": util.parse(self.collectionBuilder.Type, "year", dict_data, datatype="int", methods=dict_methods, default=self.collectionBuilder.current_year, parent=method_name, minimum=1917, maximum=self.collectionBuilder.current_year + 1),
                        "limit": util.parse(self.collectionBuilder.Type, "limit", dict_data, datatype="int", methods=dict_methods, default=100, parent=method_name, maximum=500),
                        "starting_only": util.parse(self.collectionBuilder.Type, "starting_only", dict_data, datatype="bool", methods=dict_methods, default=False, parent=method_name)
                    }))
                elif method_name == "mal_userlist":
                    self.collectionBuilder.builders.append((method_name, {
                        "username": util.parse(self.collectionBuilder.Type, "username", dict_data, methods=dict_methods, parent=method_name),
                        "status": util.parse(self.collectionBuilder.Type, "status", dict_data, methods=dict_methods, parent=method_name, default="all", options=mal.userlist_status),
                        "sort_by": util.parse(self.collectionBuilder.Type, "sort_by", dict_data, methods=dict_methods, parent=method_name, default="score", options=mal.userlist_sort_options, translation=mal.userlist_sort_translation),
                        "limit": util.parse(self.collectionBuilder.Type, "limit", dict_data, datatype="int", methods=dict_methods, default=100, parent=method_name, maximum=1000)
                    }))
                elif method_name == "mal_search":
                    final_attributes = {}
                    final_text = "MyAnimeList Search"
                    if "sort_by" in dict_methods:
                        sort = util.parse(self.collectionBuilder.Type, "sort_by", dict_data, methods=dict_methods, parent=method_name, options=mal.search_combos)
                    ...
                    if "limit" in dict_methods:
                        limit = util.parse(self.collectionBuilder.Type, "limit", dict_data, datatype="int", default=0, methods=dict_methods, parent=method_name)
                        final_text += f"\nLimit: {limit if limit else 'None'}"
                    if "query" in dict_methods:
                        final_attributes["q"] = util.parse(self.collectionBuilder.Type, "query", dict_data, methods=dict_methods, parent=method_name)
                        final_text += f"\nQuery: {final_attributes['q']}"
                    if "prefix" in dict_methods:
                        final_attributes["letter"] = util.parse(self.collectionBuilder.Type, "prefix", dict_data, methods=dict_methods, parent=method_name)
                        final_text += f"\nPrefix: {final_attributes['letter']}"
                    if "type" in dict_methods:
                        type_list = util.parse(self.collectionBuilder.Type, "type", dict_data, datatype="commalist", methods=dict_methods, parent=method_name, options=mal.search_types)
                        final_attributes["type"] = ",".join(type_list)
                        final_text += f"\nType: {' or '.join(type_list)}"
                    if "status" in dict_methods:
                        final_attributes["status"] = util.parse(self.collectionBuilder.Type, "status", dict_data, methods=dict_methods, parent=method_name, options=mal.search_status)
                        final_text += f"\nStatus: {final_attributes['status']}"
                    if "genre" in dict_methods:
                        genre_str = str(util.parse(self.collectionBuilder.Type, "genre", dict_data, methods=dict_methods, parent=method_name))
                        out_text, out_ints = util.parse_and_or(self.collectionBuilder.Type, 'Genre', genre_str, self.collectionBuilder.config.MyAnimeList.genres)
                        final_text += f"\nGenre: {out_text}"
                        final_attributes["genres"] = out_ints
                    if "genre.not" in dict_methods:
                        genre_str = str(util.parse(self.collectionBuilder.Type, "genre.not", dict_data, methods=dict_methods, parent=method_name))
                        out_text, out_ints = util.parse_and_or(self.collectionBuilder.Type, 'Genre', genre_str, self.collectionBuilder.config.MyAnimeList.genres)
                        final_text += f"\nNot Genre: {out_text}"
                        final_attributes["genres_exclude"] = out_ints
                    if "studio" in dict_methods:
                        studio_str = str(util.parse(self.collectionBuilder.Type, "studio", dict_data, methods=dict_methods, parent=method_name))
                        out_text, out_ints = util.parse_and_or(self.collectionBuilder.Type, 'Studio', studio_str, self.collectionBuilder.config.MyAnimeList.studios)
                        final_text += f"\nStudio: {out_text}"
                        final_attributes["producers"] = out_ints
                    if "content_rating" in dict_methods:
                        final_attributes["rating"] = util.parse(self.collectionBuilder.Type, "content_rating", dict_data, methods=dict_methods, parent=method_name, options=mal.search_ratings)
                        final_text += f"\nContent Rating: {final_attributes['rating']}"
                    if "score.gte" in dict_methods:
                        final_attributes["min_score"] = util.parse(self.collectionBuilder.Type, "score.gte", dict_data, datatype="float", methods=dict_methods, parent=method_name, minimum=0, maximum=10)
                        final_text += f"\nScore Greater Than or Equal: {final_attributes['min_score']}"
                    elif "score.gt" in dict_methods:
                        original_score = util.parse(self.collectionBuilder.Type, "score.gt", dict_data, datatype="float", methods=dict_methods, parent=method_name, minimum=0, maximum=10)
                        final_attributes["min_score"] = original_score + 0.01
                        final_text += f"\nScore Greater Than: {original_score}"
                    if "score.lte" in dict_methods:
                        final_attributes["max_score"] = util.parse(self.collectionBuilder.Type, "score.lte", dict_data, datatype="float", methods=dict_methods, parent=method_name, minimum=0, maximum=10)
                        final_text += f"\nScore Less Than or Equal: {final_attributes['max_score']}"
                    elif "score.lt" in dict_methods:
                        original_score = util.parse(self.collectionBuilder.Type, "score.lt", dict_data, datatype="float", methods=dict_methods, parent=method_name, minimum=0, maximum=10)
                    ...
                    if "min_score" in final_attributes and "max_score"  in final_attributes and final_attributes["max_score"] <= final_attributes["min_score"]:
                        raise Failed(f"{self.collectionBuilder.Type} Error: mal_search score.lte/score.lt attribute must be greater than score.gte/score.gt")
                    if "sfw" in dict_methods:
                        sfw = util.parse(self.collectionBuilder.Type, "sfw", dict_data, datatype="bool", methods=dict_methods, parent=method_name)
                        if sfw:
                            final_attributes["sfw"] = 1
                            final_text += f"\nSafe for Work: {final_attributes['sfw']}"
                    if not final_attributes:
                        raise Failed(f"{self.collectionBuilder.Type} Error: no mal_search attributes found")
            ...
            item_list = util.parse(self.collectionBuilder.Type, method_name[4:], method_data, datatype="commalist")
    ...
    def _mojo(self, method_name, method_data):
        for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
            ...
            if method_name == "mojo_record":
                final["chart"] = util.parse(self.collectionBuilder.Type, "chart", dict_data, methods=dict_methods, parent=method_name, options=mojo.top_options)
            elif method_name == "mojo_world":
                if "year" not in dict_methods:
                    raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} year attribute not found")
                og_year = dict_data[dict_methods["year"]]
                if not og_year:
                    raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} year attribute is blank")
                if og_year == "current":
                    ...
                    try:
                        ...
                        if final["year"] not in mojo.year_options:
                            raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} year attribute final value must be 1977 or greater: {og_year}")
                    except ValueError:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} year attribute invalid: {og_year}")
                else:
                    final["year"] = util.parse(self.collectionBuilder.Type, "year", dict_data, methods=dict_methods, parent=method_name, options=mojo.year_options)
            elif method_name == "mojo_all_time":
                final["chart"] = util.parse(self.collectionBuilder.Type, "chart", dict_data, methods=dict_methods, parent=method_name, options=mojo.chart_options)
                final["content_rating_filter"] = util.parse(self.collectionBuilder.Type, "content_rating_filter", dict_data, methods=dict_methods, parent=method_name, options=mojo.content_rating_options) if "content_rating_filter" in dict_methods else None
            elif method_name == "mojo_never":
                final["chart"] = util.parse(self.collectionBuilder.Type, "chart", dict_data, methods=dict_methods, parent=method_name, default="domestic", options=self.collectionBuilder.config.BoxOfficeMojo.never_options)
                final["never"] = str(util.parse(self.collectionBuilder.Type, "never", dict_data, methods=dict_methods, parent=method_name, default="1", options=mojo.never_in_options)) if "never" in dict_methods else "1"
            elif method_name in ["mojo_domestic", "mojo_international"]:
                dome = method_name == "mojo_domestic"
                final["range"] = util.parse(self.collectionBuilder.Type, "range", dict_data, methods=dict_methods, parent=method_name, options=mojo.dome_range_options if dome else mojo.intl_range_options)
                if not dome:
                    final["chart"] = util.parse(self.collectionBuilder.Type, "chart", dict_data, methods=dict_methods, parent=method_name, default="international", options=self.collectionBuilder.config.BoxOfficeMojo.intl_options)
                ...
                if final["range"] != "daily":
                    ...
                    if _m not in dict_methods:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {_m} attribute not found")
                    og_year = dict_data[dict_methods[_m]]
                    if not og_year:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {_m} attribute is blank")
                    if str(og_year).startswith("current-"):
                        try:
                            chart_date = self.collectionBuilder.current_time - relativedelta(years=int(og_year.split("-")[1]))
                        except ValueError:
                            raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {_m} attribute invalid: {og_year}")
                    else:
                        _y = util.parse(self.collectionBuilder.Type, _m, dict_data, methods=dict_methods, parent=method_name, default="current", options=mojo.year_options)
                        if _y != "current":
                ...
                if final["range"] != "yearly":
                    if "range_data" not in dict_methods:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} range_data attribute not found")
                    og_data = dict_data[dict_methods["range_data"]]
                    if not og_data:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} range_data attribute is blank")

                    if final["range"] == "holiday":
                        final["range_data"] = util.parse(self.collectionBuilder.Type, "range_data", dict_data, methods=dict_methods, parent=method_name, options=mojo.holiday_options)
                        ...
                        if og_data == "current":
                            ...
                            try:
                                ...
                                raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} range_data attribute invalid: {og_data}")
                        else:
                            final["range_data"] = util.parse(self.collectionBuilder.Type, "range_data", dict_data, methods=dict_methods, parent=method_name, default="current", datatype="date", date_return="%Y-%m-%d")
                        ...
                        if str(og_data).startswith("current-"):
                            ...
                            _v = util.parse(self.collectionBuilder.Type, "range_data", dict_data, methods=dict_methods, parent=method_name, default="current", options=["current"] + [str(i) for i in range(1, 54)])
                        ...
                        if str(og_data).startswith("current-"):
                            ...
                            _v = util.parse(self.collectionBuilder.Type, "range_data", dict_data, methods=dict_methods, parent=method_name, default="current", options=["current"] + util.lower_months)
                        ...
                        if str(og_data).startswith("current-"):
                            ...
                            _v = util.parse(self.collectionBuilder.Type, "range_data", dict_data, methods=dict_methods, parent=method_name, default="current", options=mojo.quarter_options)
                            final["range_data"] = mojo.quarters[chart_date.month] if _v == "current" else _v
                    elif final["range"] == "season":
                        _v = util.parse(self.collectionBuilder.Type, "range_data", dict_data, methods=dict_methods, parent=method_name, default="current", options=mojo.season_options)
                ...
                if final["year"] < 1977:
                    raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} attribute final date value must be on year 1977 or greater: {final['year']}")

            final["limit"] = util.parse(self.collectionBuilder.Type, "limit", dict_data, methods=dict_methods, parent=method_name, default=0, datatype="int", maximum=1000) if "limit" in dict_methods else 0
    ...
    def _plex(self, method_name, method_data):
        if method_name in ["plex_all", "plex_pilots"]:
            ...
            if method_data not in plex.watchlist_sorts:
                self.logger.warning(f"{self.collectionBuilder.Type} Warning: Watchlist sort: {method_data} invalid defaulting to added.asc")
            ...
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                ...
                if method_name == "plex_search":
                    ...
                    prefix_list = util.parse(self.collectionBuilder.Type, "exclude_prefix", dict_data, datatype="list", methods=dict_methods) if "exclude_prefix" in dict_methods else []
                    exact_list = util.parse(self.collectionBuilder.Type, "exclude", dict_data, datatype="list", methods=dict_methods) if "exclude" in dict_methods else []
                    if len(prefix_list) == 0 and len(exact_list) == 0:
                        raise Failed(f"{self.collectionBuilder.Type} Error: you must have at least one exclusion")
    ...
    def _reciperr(self, method_name, method_data):
        if method_name == "reciperr_list":
            ...
            self.collectionBuilder.builders.append((method_name, util.parse(self.collectionBuilder.Type, method_name, method_data, "bool")))

    def _mdblist(self, method_name, method_data):
        for mdb_dict in self.collectionBuilder.config.MDBList.validate_mdblist_lists(self.collectionBuilder.Type, method_data):
    ...
    def _tautulli(self, method_name, method_data):
        for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
            ...
            final_dict = {
                ...
                "list_days": util.parse(self.collectionBuilder.Type, "list_days", dict_data, datatype="int", methods=dict_methods, default=30, parent=method_name),
                "list_size": util.parse(self.collectionBuilder.Type, "list_size", dict_data, datatype="int", methods=dict_methods, default=10, parent=method_name),
                "list_minimum": util.parse(self.collectionBuilder.Type, "list_minimum", dict_data, datatype="int", methods=dict_methods, default=0, parent=method_name)
            }
            buff = final_dict["list_size"] * 3
            if self.collectionBuilder.library.Tautulli.has_section:
                buff = 0
            elif "list_buffer" in dict_methods:
                buff = util.parse(self.collectionBuilder.Type, "list_buffer", dict_data, datatype="int", methods=dict_methods, default=buff, parent=method_name)
    ...
    def _tmdb(self, method_name, method_data):
        if method_name == "tmdb_discover":
            for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
                dict_methods = {dm.lower(): dm for dm in dict_data}
                new_dictionary = {"limit": util.parse(self.collectionBuilder.Type, "limit", dict_data, datatype="int", methods=dict_methods, default=100, parent=method_name)}
                for discover_method, discover_data in dict_data.items():
                    ...
                    if discover_data is None:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_method} attribute is blank")
                    elif discover_method.lower() not in tmdb.discover_all:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_method} attribute not supported")
                    elif self.collectionBuilder.library.is_movie and discover_attr in tmdb.discover_tv_only:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_method} attribute only works for show libraries")
                    elif self.collectionBuilder.library.is_show and discover_attr in tmdb.discover_movie_only:
                        raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_method} attribute only works for movie libraries")
                    elif discover_attr == "region":
                        new_dictionary[discover_attr] = util.parse(self.collectionBuilder.Type, discover_method, discover_data.upper(), parent=method_name, regex=("^[A-Z]{2}$", "US"))
                        ...
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, parent=method_name, options=options)
                        ...
                        if "certification" in dict_data or "certification.lte" in dict_data or "certification.gte" in dict_data:
                            ...
                            raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_attr} attribute: must be used with either certification, certification.lte, or certification.gte")
                    elif discover_attr == "certification":
                        if "certification_country" in dict_data:
                            new_dictionary[lower_method] = discover_data
                        else:
                            raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_method} attribute: must be used with certification_country")
                        ...
                        if "with_watch_providers" in dict_data or "without_watch_providers" in dict_data or "with_watch_monetization_types" in dict_data:
                            ...
                            raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_method} attribute: must be used with either with_watch_providers, without_watch_providers, or with_watch_monetization_types")
                    elif discover_attr == "with_watch_monetization_types":
                        if "watch_region" in dict_data:
                            new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, parent=method_name, options=tmdb.discover_monetization_types)
                        else:
                            raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} {discover_method} attribute: must be used with watch_region")
                    elif discover_attr in tmdb.discover_booleans:
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, datatype="bool", parent=method_name)
                    elif discover_attr == "vote_average":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, datatype="float", parent=method_name)
                    elif discover_attr == "with_status":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, datatype="int", parent=method_name, minimum=0, maximum=5)
                    elif discover_attr == "with_type":
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, datatype="int", parent=method_name, minimum=0, maximum=6)
                    elif discover_attr in tmdb.discover_dates:
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, datatype="date", parent=method_name, date_return="%m/%d/%Y")
                    elif discover_attr in tmdb.discover_years:
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, datatype="int", parent=method_name, minimum=1800, maximum=self.collectionBuilder.current_year + 1)
                    elif discover_attr in tmdb.discover_ints:
                        new_dictionary[lower_method] = util.parse(self.collectionBuilder.Type, discover_method, discover_data, datatype="int", parent=method_name)
    ...
    def _trakt(self, method_name, method_data):
        if method_name.startswith("trakt_list"):
            ...
            if util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="bool", default=False):
                ...
                raise Failed(f"{self.collectionBuilder.Type} Error: {method_name} must be set to true")
        elif method_name == "trakt_recommendations":
            self.collectionBuilder.builders.append((method_name, util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", default=10, maximum=100)))
        elif method_name == "sync_to_trakt_list":
            if method_data not in self.collectionBuilder.config.Trakt.slugs:
                raise Failed(f"{self.collectionBuilder.Type} Error: {method_data} invalid. Options {', '.join(self.collectionBuilder.config.Trakt.slugs)}")
            self.collectionBuilder.sync_to_trakt_list = method_data
        elif method_name == "sync_missing_to_trakt_list":
            self.collectionBuilder.sync_missing_to_trakt_list = util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="bool", default=False)
            ...
            if method_name in ["trakt_chart", "trakt_userlist"]:
                ...
                trakt_dicts = {
                    "chart": terms[1],
                    "limit": util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="int", default=10),
                    "time_period": terms[2] if len(terms) > 2 else None
                }
                final_method = "trakt_chart"
            if method_name != final_method:
                self.logger.warning(f"{self.collectionBuilder.Type} Warning: {method_name} will run as {final_method}")
            for trakt_dict in self.collectionBuilder.config.Trakt.validate_chart(self.collectionBuilder.Type, final_method, trakt_dicts, self.collectionBuilder.library.is_movie):
        ...
        for dict_data in util.parse(self.collectionBuilder.Type, method_name, method_data, datatype="listdict"):
            ...
            if "validate" in dict_methods:
                if dict_data[dict_methods["validate"]] is None:
                    raise Failed(f"{self.collectionBuilder.Type} Error: validate filter attribute is blank")
                if not isinstance(dict_data[dict_methods["validate"]], bool):
                    raise Failed(f"{self.collectionBuilder.Type} Error: validate filter attribute must be either true or false")
            ...
            for filter_method, filter_data in dict_data.items():
                ...
                if filter_final not in all_filters:
                    message = f"{self.collectionBuilder.Type} Error: {filter_final} is not a valid filter attribute"
                elif self.collectionBuilder.builder_level in filters and filter_attr not in filters[self.collectionBuilder.builder_level]:
                    message = f"{self.collectionBuilder.Type} Error: {filter_final} is not a valid {self.collectionBuilder.builder_level} filter attribute"
                elif filter_final is None:
                    message = f"{self.collectionBuilder.Type} Error: {filter_final} filter attribute is blank"
                else:
                    try:
