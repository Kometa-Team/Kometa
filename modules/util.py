...
def load_files(files_to_load, method, err_type="Config", schedule=None, lib_vars=None, single=False, config_dir=None):
    def _resolve(path):
        """Return an absolute path, anchoring relative paths to config_dir when provided."""
        ...
        if isinstance(file, dict):
            ...
            if not single and "folder" in file:
                ...
                if file["folder"] is None:
                    ...
                    folder_path = _resolve(file["folder"])
            ...
            if "asset_directory" in file and file["asset_directory"]:
                ...
                for asset_path in get_list(file["asset_directory"], split=False):
                    resolved_asset = _resolve(asset_path)
            ...
            if schedule and "schedule" in file and file["schedule"]:
                ...
                if err:
                    ...
                    continue
            # Resolve relative File paths to absolute so downstream code (meta.py) never
            # has to guess the base directory.
            resolved_current = [(ft, _resolve(fp) if ft == "File" else fp) for ft, fp in current]
            files.extend([(ft, fp, temp_vars, asset_directory) for ft, fp in resolved_current])
        else:
            logger.info(f"Reading file: {file}")
            resolved_file = _resolve(file)
