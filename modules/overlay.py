...
class Overlay:
    def __init__(self, config, library, overlay_file, original_mapping_name, overlay_data, suppress, level):
        ...
        if "name" not in self.data or not self.data["name"]:
            raise OverlayError(f"Overlay Error: overlay must have the 'name' attribute")
        ...
        if "queue" in self.data and self.data["queue"]:
            ...
            if self.queue_name not in self.overlay_file.queue_names:
                raise OverlayError(f"Overlay Error: queue '{self.queue_name}' not found")
            ...
            if "font" in self.data and self.data["font"]:
                ...
                if not os.path.exists(font):
                    ...
                    if font not in fonts:
                        raise OverlayError(f"Overlay Error: Font '{os.path.abspath(font)}' not found. Available fonts: {', '.join(fonts)}")
            ...
            if "font_style" in self.data and self.data["font_style"]:
                try:
                    ...
                    if self.data["font_style"] in variation_names:
                        ...
                        raise OverlayError(f"Overlay Error: Font style '{self.data['font_style']}' not found. Available font styles: {','.join(variation_names)}")
                except OSError:
            ...
            if "<<originally_available[" in text:
                ...
                if match:
                    try:
                        ...
                        raise OverlayError("Overlay Error: 'originally_available' date format not valid")
