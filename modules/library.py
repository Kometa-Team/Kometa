...
class Library(ABC):
    ...
    def upload_images(self, item, poster=None, background=None, logo=None, square=None, overlay=False):
        ...
        if square is not None:
            try:
                image_compare = None
                if self.config.Cache:
                    _, image_compare, _ = self.config.Cache.query_image_map(item.ratingKey, f"{self.image_table_name}_squares")
                if not image_compare or str(square.compare) != str(image_compare):
                    square_uploaded = self._upload_image(item, square)
                    logger.info(f"Metadata: {square.attribute} updated {square.message}")
                elif self.show_asset_not_needed:
                    logger.info(f"Metadata: {square.prefix}square update not needed")
            except Failed:
                logger.stacktrace()
                logger.error(f"Metadata: {square.attribute} failed to update {square.message}")
        ...
        if self.config.Cache:
            ...
            if square_uploaded:
                self.config.Cache.update_image_map(item.ratingKey, f"{self.image_table_name}_squares", "", square.compare)

        return poster_uploaded, background_uploaded, logo_uploaded, square_uploaded
