...
class BoxOfficeMojo:
    ...
    def _imdb(self, url):
        ...
        if not imdb_url:
            raise Failed(f"[BLE00039] BoxOfficeMojo Error: IMDb ID not found at {base_url}{url}")
    ...
    def get_imdb_ids(self, method, data):
        ...
        if not items:
            raise Failed(f"[BLE00040] BoxOfficeMojo Error: No List Items found in {method}: {data}")
        ids = []
