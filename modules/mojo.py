...
class BoxOfficeMojo:
    ...
    def _imdb(self, url):
        ...
        if not imdb_url:
            raise Failed(f"[B114] Builder Error: BoxOfficeMojo returned no IMDb ID for {base_url}{url}")
    ...
    def get_imdb_ids(self, method, data):
        ...
        if not items:
            raise Failed(f"[B115] Builder Error: No BoxOfficeMojo list items found in {method}: {data}")
        ids = []
