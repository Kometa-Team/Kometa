...
class Letterboxd:
    ...
    def _tmdb(self, letterboxd_url, language):
        ...
        if len(ids) > 0 and ids[0]:
            ...
            raise Failed(f"[BLE0022] Letterboxd Error: TMDb Movie ID not found in {ids[0]}")
        raise Failed(f"[BLE0023] Letterboxd Error: TMDb Movie ID not found at {letterboxd_url}")
    ...
    def get_tmdb_ids(self, method, data, language):
        if method == "letterboxd_list":
            ...
            if total_items > 0:
                ...
                raise Failed(f"[BLE0024] Letterboxd Error: No List Items found in {data}")
        else:
            raise Failed(f"[BLE0025] Letterboxd Error: Method {method} not supported")
