...
class Notifiarr:
    ...
    def _request(self, json=None, path="notification", params=None):
        ...
        try:
            ...
            logger.error(e)
            raise Failed("[S217] Connector Error: Invalid Notifiarr JSON response received")
        if response.status_code >= 400 or ("result" in response_json and response_json["result"] == "error"):
            logger.debug(f"Response: {response_json}")
            raise Failed(f"({response.status_code} [{response.reason}]) {response_json}")
        if not response_json["details"]["response"]:
            raise Failed("[S218] Connector Error: Notifiarr API key is invalid. Please check the API key is correct")
        return response
