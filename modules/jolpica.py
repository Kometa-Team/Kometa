...
class Jolpica:
    ...
    def get_races(self, year, language, ignore_cache=False):
        ...
        if self.cache and not ignore_cache:
            race_list, expired = self.cache.query_jolpica(year, self.cache.expiration)
        ...
        if response.status_code < 400:
            ...
            if self.cache and not ignore_cache:
                self.cache.update_jolpica(expired, year, races, self.cache.expiration)
            return races
        else:
            raise Failed(f"Jolpica Error: F1 Season: {year} Not found")
