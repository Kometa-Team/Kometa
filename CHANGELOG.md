...
- `modules/imdb.py`: IMDb-backed mass rating, genre, and episode-rating lookups now prefer the Kometa IMDb Service (`utilities.kometa.wiki/imdb-service`). If the service is unresponsive, Kometa logs the failure and falls back to downloading the public IMDb TSV datasets (`title.ratings.tsv.gz`, `title.basics.tsv.gz`, `title.episode.tsv.gz`).
- `modules/imdb.py`: parental guide lookups now use the Kometa IMDb Service exclusively; the client-side GraphQL fallback has been removed because the service now provides that fallback internally.
