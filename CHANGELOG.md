...
- `json-schema/config-schema.json`: add missing `^exclude_users_.+$` pattern property to `definitions.template-variables-playlists.patternProperties`. Kometa's own `defaults/playlist.yml` already emits keyed overrides via `exclude_users_<<key>>: <<exclude_users>>` (line 62), and every sibling in the same family (`^sync_to_users_.+$`, `^name_.+$`, `^summary_.+$`, `^radarr_add_missing_.+$`, `^exclude_user_.+$` singular, etc.) has a keyed counterpart. Config validators (Quickstart, yaml-language-server, any tooling using this schema) were flagging valid `exclude_users_<key>: [...]` overrides as errors. New pattern mirrors the sibling `^sync_to_users_.+$` shape (`oneOf: [string, array]`).
