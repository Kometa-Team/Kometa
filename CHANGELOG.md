...
- Update requirements
- `modules/imdb.py`: the `imdb_search` `interests:` catalog (interest name → IMDb `in########` id) is now fetched at runtime from `Kometa-Team/IMDb-Hash` (`INTERESTS.json`) and cached per run, with the bundled snapshot used only as a fallback when the fetch fails. This lets new IMDb interests become usable without a Kometa release. The bundled fallback is also refreshed from 211 to the current 313 interests. A staged generator script and scheduled workflow (`.github/imdb-hash-repo-files/`) keep `INTERESTS.json` up to date in the IMDb-Hash repo.
