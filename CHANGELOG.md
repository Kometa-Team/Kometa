...
- Fix relative paths in `collection_files`, `metadata_files`, `overlay_files`, `playlist_files`, `image_files`, `folder`, and `asset_directory` config entries resolving against the process working directory instead of the config file's directory when a custom config path is provided via `-c`. Fixes #3092.
